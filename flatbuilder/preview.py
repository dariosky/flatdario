...
def page_not_found(e):
    return "Page not found - this will be the 404 you'll configure on your webserver", 404
...
def serve(path, port=7747):
    ...
    logger.info("Running preview of {path} on http://localhost:{port}".format(path=path,
                                                                              port=port))
