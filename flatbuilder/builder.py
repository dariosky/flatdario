...
class Builder:
    ...
    def init(self, template_folder):
        ...
        if os.path.isdir(self.build_folder):
            ...
            response = input(
                "Do you want to copy the template %s over"
    ...
    def run(self, items, folder):
        ...
        try:
            ...
            logger.error(
                "Build folder doesn't seem coming from a template, we need %s"
                % CONFIG_FILENAME
            )
            return
    ...
    def process(self, items):
        ...
        for page in self.config["pages"]:
            path = page["path"].lstrip(
                "/"
            )  # strip absolute path, we are relative to build
            ...
            if not os.path.isfile(file_path):
                logger.debug(
                    "File missing [%s] creating an empty one with just the placeholder."
                    % path
                )
                ...
                with open(file_path, "w") as f:
                    assert selector[0] in (".", "#"), (
                        "I was expecting a class or id selector"
                    )
                    selector_name = "id" if selector[0] == "#" else "class"
                    selector_value = selector[1:]
                    f.write(
                        "<html><body><div {attr}='{value}'></div></body></html>".format(
