...
class YouTubeLikesCollector(Collector):
    ...
    def get_credentials(self):
        flow = flow_from_clientsecrets(
            ...
            message="You need the %s file with your app credentials"
            % self.CLIENT_SECRETS_FILE,
    ...
    def run(self, **params):
        ...
        for channel in channels_response["items"]:
            ...
            playlistitems_list_request = youtube.playlistItems().list(
                playlistId=list_id, part="snippet,status", maxResults=5
            )
            ...
            while playlistitems_list_request:
                ...
                for playlist_item in playlistitems_list_response["items"]:
                    if playlist_item["status"]["privacyStatus"] in (
                        "unlisted",
                        "private",
                    ):
                        # ignore unlisted
                    ...
                    if "thumbnails" not in snippet:
                        logger.debug(
                            "Skipping video {title} ({id})".format(
