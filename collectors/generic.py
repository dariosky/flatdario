...
class OAuthCollector(Collector):
    ...
    def get_api_secrets(self):
        ...
        try:
            ...
            logger.error(
                f"Cannot read the API secrets for {self.type} in {api_secrets_file}."
            )
            raise
        else:
            return api_secrets
    ...
    def get_endpoints(self):
        ...
        raise NotImplementedError(
            "Collectors should define the endpoints for oAuth.\n"
    ...
    def start_local_server(hostname="localhost", ports=(8080, 8090)):
        for port in ports:
            try:
                httpd = ClientRedirectServer((hostname, port), ClientRedirectHandler)
            except socket.error:
                pass
            else:
                return httpd
    ...
    def authenticate(self):
        ...
        if access_token is None:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Error getting data from f{self.type}: %s" % response.text
                )
                logger.error(response.headers.get("X-Error", f"{self.type} error"))
