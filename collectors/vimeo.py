...
class VimeoCollector(OAuthCollector):
    ...
    def authenticate(self):
        ...
        auth = dict(key=api_secrets["clientID"], secret=api_secrets["clientSecrets"])
        if access_token:
            auth["token"] = access_token
        self.vimeo = vimeo.VimeoClient(**auth)
        if not access_token:
            import webbrowser
            ...
            if server is None:
                ...
                logger.warning(
                    "Unable to start a localhost webserver to receive the response."
                )
                logger.info(
                ...
                authorize_url = self.vimeo.auth_url(
                    ["public", "private"], redirect_uri, None
                )

                webbrowser.open(authorize_url, new=1, autoraise=True)
                logger.info(
                    "Opened browser to authorize access: ".format(address=authorize_url)
                )
                server.handle_request()
                token = server.query_params["code"]
                try:
                    access_token, user, scope = self.vimeo.exchange_code(
                        token, redirect_uri
                    )
                except vimeo.auth.GrantFailed:
                    raise
                self.save_user_secrets(dict(access_token=access_token))
                ...
                try:
                    ...
                    logger.info(
                        "{type} - {title} ({id})".format(
                            type=self.type, title=title, id=video_id
                        )
                    )
                    count += 1
...
def parse_datetime(timestamp):
    return datetime.datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S+00:00")
